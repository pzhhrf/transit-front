    ...
    <div class="in">
      ...
      <div class="in_05">
        ...
        >
          ...
          <el-table-column label="提取状态" align="center" width="140">
            <template slot-scope="{ row }">
              <span>{{ row.status == 1 ? "提取成功" : "提取失败" }}</span>
            </template>
          </el-table-column>
          <el-table-column label="原始路径" align="center" width="150">
            <template slot-scope="{ row }">
        ...
        <div class="content">
          ...
          >
            ...
            <el-table-column label="转存时间" align="center">
              <template slot-scope="{ row }">
                <span>{{ row.complete_time | filterTime }}</span>
              </template>
            </el-table-column>
            ...
            <el-table-column label="下载地址" align="center">
              <template slot-scope="{ row }">
                <span v-for="item in row.download_list" :key="item.id">
                  <el-button
                    type="primary"
                    size="mini"
  ...
  methods: {
    ...
    getTasks() {
      ...
      request.getTasks(dict).then((res) => {
        if (res.code == 0) {
          this.downData = this.dealDownData(res.data && res.data.content);
        }
      });
    },
    getSpeedInfo(id) {
